...
module.exports = class VulcanGenerator extends Generator {
  constructor(args, options) {
    ...
    if (!store) {
      ...
      if (process.env.VULCANJS_SEE_REDUX_LOGS) {
        store = Redux.createStore(reducers, allConfig, Redux.applyMiddleware(logger()));
      } else {
        store = Redux.createStore(reducers, allConfig);
      }
    }
  ...
  */
  ...
  _getRootStoriesPath(options, ...args) {
    return this._getPath(options, '.stories', ...args);
  }

  _getPackagePath(options, ...args) {
    return this._getPath(options, 'packages', this.props.packageName, ...args);
  }

  _getLibPath(options, ...args) {
    return this._getPackagePath(options, 'lib', ...args);
  }

  _getModulesPath(options, ...args) {
    return this._getLibPath(options, 'modules', ...args);
  }

  _getModulePath(options, ...args) {
    return this._getModulesPath(options, this.props.moduleName, ...args);
  }

  _getComponentsPath(options, ...args) {
    return this._getLibPath(options, 'components', ...args);
  }

  _getModuleTestPath(options, ...args) {
    return this._getModulePath(options, 'test', ...args);
  }

  _getPackageStoriesPath(options, ...args) {
    return this._getComponentsPath(options, '.stories.js', ...args);
  }

  _getModuleInComponentsPath(options, ...args) {
    return this._getComponentsPath(options, this.props.moduleName, ...args);
  }

  _getComponentPath(options) {
    return this._getModuleInComponentsPath(options, this._getComponentFileName());
  }

  _getModuleStoriesPath(options) {
    return this._getModuleInComponentsPath(options, '.stories.js');
  }

  _getClientPath(options, ...args) {
    return this._getLibPath(options, 'client', ...args);
  }

  _getServerPath(options, ...args) {
    return this._getLibPath(options, 'server', ...args);
  }
  ...
  */
  ...
  _getPackageNameListQuestion() {
    return {
      ...
      default: common.getDefaultChoiceIndex(this._getPackageNames(), this.options.packagename)
    };
  }
  ...
  _getModuleNameListQuestion() {
    return {
      ...
      default: answers => {
        const packageName = this._getFinalPackageName(answers);
        return common.getDefaultChoiceIndex(this._getModuleNames(packageName), this.options.moduleName);
      }
    };
  }
  ...
  _getStoryBookSetupQuestion() {
    return {
      ...
      choices: [{ name: 'Yes, take me to storybook setup after this.', value: 'installing' }, { name: 'No, ask later.', value: 'pending' }, { name: 'No, and dont ask again.', value: 'dontask' }],
      when: () => {
  ...
  _getAllCollectionPropertyQuestions() {
    return [this._getCollectionPropertyNameQuestion(), this._getIsCollectionPropertyHiddenQuestion(), this._getCollectionPropertyLabelQuestion(), this._getCollectionPropertyTypeQuestion(), this._getIsCollectionPropertyOptionalQuestion(), this._getCollectionPropertyViewableByQuestion(), this._getCollectionPropertyInsertableByQuestion(), this._getCollectionPropertyEditableByQuestion(), this._getIsAddAnotherCustomCollectionPropertyQuestion()];
  }
  ...
  */
  ...
  _getFinalPackageName(answers) {
    ...
    return this._filterPackageName(packageName || answers.packageName);
  }
...
//# sourceMappingURL=VulcanGenerator.js.map
